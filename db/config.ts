...
export const config = {
	app: {
		...
		frontendBaseUrl: getEnv(string("FRONTEND_BASE_URL")).default(
			"http://localhost:5173",
		),
	...
	postgres: {
		...
		migrationsFolder: getEnv(string("MIGRATIONS_FOLDER")).default(
			"../migrations",
		),
	},
	...
	minio: {
		...
		storageUrl: getEnv(string("STORAGE_URL")).default(
			"http://localhost:9000/",
		),
